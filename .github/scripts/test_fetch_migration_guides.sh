#!/bin/bash
# test_fetch_migration_guides.sh
set -euo pipefail

PROJECT_ROOT="${1:-.}"
PROJECT_ROOT="$(cd "$PROJECT_ROOT" && pwd)"
cd "$PROJECT_ROOT"

CMAKE_FILE="CMakeLists.txt"
MEM_FILE="memory/migrations.md"
SKILL_SCRIPTS=".agents/skills/az-upgrade-api/scripts"

# Save original version so we can restore it.
ORIGINAL_VER=$("./$SKILL_SCRIPTS/parse_current_api_ver.sh")

# Ensure CMakeLists.txt is restored on exit, even on failure.
restore_version() {
    perl -0777 -pi -e "s/(ALPHA_ZERO_API_GIT_TAG\s+)\"[^\"]+\"/\$1\"$ORIGINAL_VER\"/" "$CMAKE_FILE"
}
trap restore_version EXIT

# Change ALPHA_ZERO_API_GIT_TAG in CMakeLists.txt to v0.0.0.
# perl -0777 slurps the whole file so \s+ can span newlines.
perl -0777 -pi -e 's/(ALPHA_ZERO_API_GIT_TAG\s+)"[^"]+"/$1"v0.0.0"/' "$CMAKE_FILE"

CURRENT_VER=$("./$SKILL_SCRIPTS/parse_current_api_ver.sh")
if [ "$CURRENT_VER" != "v0.0.0" ]; then
    echo "FAIL: Expected version v0.0.0 but got $CURRENT_VER" >&2
    exit 1
fi

# Run fetch_migration_guides.sh.
"./$SKILL_SCRIPTS/fetch_migration_guides.sh"

# Make sure memory/migrations.md has a checklist item to upgrade from v0.0.0.
if grep -q '^- \[ \] \[`v0\.0\.0` to `v' "$MEM_FILE"; then
    echo "PASS: Found migration checklist item from v0.0.0"
else
    echo "FAIL: No migration checklist item from v0.0.0 found in $MEM_FILE" >&2
    exit 1
fi

echo "PASS: All tests passed (version will be restored to $ORIGINAL_VER)"
