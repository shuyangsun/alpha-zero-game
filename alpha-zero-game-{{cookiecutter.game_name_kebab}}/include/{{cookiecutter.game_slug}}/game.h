#ifndef ALPHA_ZERO_GAME_{{cookiecutter.__include_guard_prj } } _INCLUDE_{{ \
    cookiecutter.__include_guard_slug } } _GAME_H_
#define ALPHA_ZERO_GAME_                \
  {                                     \
    {                                   \
      cookiecutter.__include_guard_prj  \
    }                                   \
  }                                     \
  _INCLUDE_ {                           \
    {                                   \
      cookiecutter.__include_guard_slug \
    }                                   \
  }                                     \
  _GAME_H_
...
}  // namespace alphazero::game::
{
  ...
  //
  // Action type: can we use a small integer type (e.g., uint8_t) to represent
  // all possible actions? What's the smallest type we can use here?
  //
  // Player type: is it a two-player game? Is it a group game? For most
  // two-player games, boolean is sufficient.
  {
    % -endif %
  }
  {
  ...
  using {{cookiecutter.__board}} =
      ::alphazero::game::api::Standard2DBoard<3, 3>;
  ...
  using {{cookiecutter.__game_interface}} =
      ::alphazero::game::api::IGame<{{cookiecutter.__board}},
  ...
  {% if cookiecutter.llm[0] | lower == 'y' -%}
     *
     * TODO(TASK-003): design constructors, then change the docstring to be
  {{cookiecutter.game_name}} specific.
     *
     * Does a default constructor make sense? Should we always explicitly pass
  in
  ...
  {%- endif %}
    ...
    {{cookiecutter.__game_cls}}(const {{cookiecutter.__game_cls}} & other) =
        default;
     ...
     *
     * @return std::unique_ptr<const {{cookiecutter.__game_interface}}> Unique
     * pointer to the new copy of the game state.
     */
     ...
     *
     * @return const {{cookiecutter.__board}}& Const reference to the current
     * game board state.
     */
    ...
    CurrentPlayer() const final;

    /**
     * @brief Get the player from last round.
     *
     * @return std::optional<{{cookiecutter.__player}}> The player from last
     * round, or std::nullopt if the game has not started yet.
     */
     ...
     *
     * @return std::optional<{{cookiecutter.__action}}> The last action taken by
     * the last player, or std::nullopt if the game has not started yet.
     */
  ...
  {% if cookiecutter.llm[0] | lower == 'y' -%}
     *
     * TODO(TASK-004): tailor this docstring to be {{cookiecutter.game_name}}
  specific, describe the canonical representation in
  ...
  {%- endif %}
     ...
     * represented as a 2D array where the current player's pieces are marked
  with
     ...
     * current player.
     *
     * This helps with training machine learning models by providing a
  consistent
     * representation of the game state from the perspective of the current
     * player during inference time.
     *
     * If variations from different perspectives are desired to increase
  variance
     * in the training data, that should be handled by augmenters. More details
  at
     * https://github.com/shuyangsun/alpha-zero-api/blob/main/src/include/alpha-zero-api/augmenter.h
     *
     * @return {{cookiecutter.__board}} The canonical representation of the
  current board state.
     */
    {
      {
    ...
    CanonicalBoard() const final;

    /**
     * @brief Check if the game is over.
     *
     * If the game is over, no more round can be played by any player. It is
     * safe to call GetScore() after the game is over.
     *
     ...
     *
     * Only guaranteed to return a meaningful score when the game is over. If
     * the game is not over, the returned score may be arbitrary and should not
     * be used for any purpose. The implementation may choose to skip game
     * status check to optimize for performance.
     *
    ...
    /**
     * @brief Returns a vector of all valid actions for the current player in
  the
     * current game state.
  {% if cookiecutter.llm[0] | lower == 'y' -%}
     *
  ...
  {%- endif %}
     ...
     * This vector should be empty if and only if the game is over. While the
  game
     ...
     *
     * @return std::vector<{{cookiecutter.__action}}> Vector of all valid
  ...
  {%- endif %}
     ...
     * ValidActions() method. The behavior is undefined if the action is
  invalid.
     * The implementation may choose to return nullptr after checking the
  validity
     ...
     *
     * @return std::unique_ptr<const {{cookiecutter.__game_interface}}> A
  pointer to the new game
     ...
     *
     * This is the most basic form of user interface for the game, should be
     * easy to understand for both human and LLM players. LLM agents may choose
     * to run the main binary in the terminal to debug game implementation.
     *
     ...
     * @return std::expected<{{cookiecutter.__action}}, std::string> The action
     * if the string is valid, or an error message if the string is invalid.
     */
     ...
     */
    std::string ActionToString(const {{cookiecutter.__action}} & action)
        const final;
   ...
   private:
    ...
    //
    // `round_`, `cur_player_`, `last_action_`, and `board_`, likely won't
    // change much, but maybe changing them can help.
    //
    ...
    // state?
    { % -endif % } uint32_t round_ = 0;
    {
      {
        cookiecutter.__board
      }
    }
    board_ = { {cookiecutter.__board} } {};
    {
      {
        cookiecutter.__player
      }
    }
    cur_player_ = { {cookiecutter.__player} } {};
