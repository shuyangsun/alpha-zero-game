...
fr_pattern='^FR-[A-Za-z0-9]+([_-][A-Za-z0-9]+)*$'
if ! [[ "$fr_id" =~ $fr_pattern ]]; then
  echo "Error: '$fr_id' is not a valid FR identifier (expected FR-... pattern)" >&2
  exit 1
fi
...
while IFS= read -r -d '' file; do
  rel="${file#"$search_dir"/}"
  if grep -qF "$fr_id" "$file"; then
    echo "Found in: $rel"
    grep -nF "$fr_id" "$file" | while IFS= read -r line; do
      echo "  $line"
    done
    found=1
  fi
