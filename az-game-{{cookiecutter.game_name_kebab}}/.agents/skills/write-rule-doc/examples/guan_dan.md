...
- Guan Dan uses two full 54-card decks, for a total of 108 cards.
...
- Only the heart copy of the current level card is wild. Because two decks are
  used, there can be at most two wild cards in play.
...
- In double tribute, one team finished first and second, so both players on
  the other team each give one card.
