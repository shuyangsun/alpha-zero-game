...
`src/main.cc` is an interactive REPL for {{cookiecutter.game_name}}. It is
the canonical way for a human to manually exercise the game implementation
end-to-end and to spot regressions across the
game / serializer / deserializer{% if cookiecutter.augmenter[0] | lower == 'y' %} / augmenter{% endif %} boundaries.
...
- After `AUGMENTATION-IMPL`: inference variant count equals
  `|`{{cookiecutter.__augmentation_enum}}`|`; training example count matches.
{%- endif %}
