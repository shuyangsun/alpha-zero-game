...
- Inference-time `Augment` returns one entry per
  `{{cookiecutter.__augmentation_enum}}` member; each variant
...
vector length, round-trip status{% if cookiecutter.augmenter[0] | lower == 'y' %}, augmentation variants{% endif %}) are the
