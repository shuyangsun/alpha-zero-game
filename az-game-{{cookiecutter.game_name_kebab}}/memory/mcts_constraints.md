...
Performance and correctness constraints the {{cookiecutter.game_name}}
...
`{{cookiecutter.__game_cls}}` is a plain value type. The engine is
templated on it directly, so:

- The compiler sees `sizeof({{cookiecutter.__game_cls}})`. Keep it
...
- No virtual methods. No `std::unique_ptr<I{{cookiecutter.__game_cls}}>`
  factory. The `static_assert(::az::game::api::Game<...>)` at the
  bottom of [game.h](../include/{{cookiecutter.game_slug}}/game.h)
  is the gate.
...
`{{cookiecutter.__game_cls}}` must be sized for the deepest rollout
the engine will run.
...
```cpp
std::array<{{cookiecutter.__action}}, {{cookiecutter.__game_cls}}::kMaxRounds.value_or(MAX_DEPTH)>
    action_history_{};
