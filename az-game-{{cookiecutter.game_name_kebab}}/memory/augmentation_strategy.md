...
How to design `{{cookiecutter.__augmentation_enum}}`,
`internal::AugmentAll`, and the inference / training augmenters for
{{cookiecutter.game_name}}. Read after [api_contract.md](./api_contract.md)
...
If {{cookiecutter.game_name}} has no exploitable board symmetry (most
...
## The `{{cookiecutter.__augmentation_enum}}` enum

In [augmentation.h](../include/{{cookiecutter.game_slug}}/augmentation.h):
...
- Order matches `internal::AugmentAll(game)` output:
  ...
  `static_cast<{{cookiecutter.__augmentation_enum}}>(i)`.
...
```cpp
[[nodiscard]] {{cookiecutter.__action}} InverseTransformAction(
    const {{cookiecutter.__action}}& augmented_action,
    {{cookiecutter.__augmentation_enum}} augmentation) noexcept;
```
...
This project was generated with `augmenter = no`. The
`{{cookiecutter.__augmentation_enum}}`, `AugmentAll`, and augmenter
classes are not part of the build, and this document does not apply.

If you later need symmetry augmentation, regenerate the project with
`augmenter = yes` and replace the generated augmentation files.
{% endif -%}
